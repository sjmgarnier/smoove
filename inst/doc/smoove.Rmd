...
```{r loading_cvm,warning=FALSE,message=FALSE}
require(smoove)
require(magrittr)
require(plyr)
require(scales)
...
```{r FirstPlot, echo=-1, fig.height=3, fig.width = 6}
...
```{r ucvm.exact, echo=-1, fig.height=4, fig.width = 6, echo = -1}
set.seed(2)
...
```{r plotRACVM, echo=-1, fig.height=4, bty="l", fig.width = 6}
...
```{r EVAFplots, echo = -1, fig.width=6, fig.height = 6}
...
```{r VAF1, fig.height=4, echo=-1, fig.width = 6}
...
```{r racvm, echo=-1, fig.height = 3, fig.width = 6}
par(bty="l", mgp = c(1.5,0.25,0), tck=0.01, las=1); plot_track(racvm$Z)
```
```{r racvm.fit, size = "footnotesize"}
...
```{r K1, echo=-1, fig.height=4, fig.width = 8}
...
```{r K1.sim, echo=-1, fig.height=4, fig.width = 8}
par(mar = c(0,2,0,0), oma = c(4,4,2,2), bty="l")
...
```{r TwoPhaseCVM, fig.height = 4, echo=-1, fig.width = 6, fig.align="center"}
par(bty="l")
...
```{r fig.height = 4, echo=-1, fig.width = 6}
par(bty="l")
...
```{r plotMulticvm, echo =-1, fig.height = 4, fig.width = 6, fig.align="center"}
...
Toggling the `progress` to TRUE will provide reports on the progress of the analysis.  There is also an option to parallelize the analysis using the `doFuture` and `foreach` R packages.  This which can speed things up considerably (roughly in proportion to the number of cores on your processor).  An example of implementation: 

```{r simSweep.parallel, warning = FALSE, eval=FALSE, message = FALSE}
...
```{r sweepKestrel, message=FALSE, eval = FALSE}
